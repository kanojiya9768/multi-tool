...
export default function LoanCalculator() {
  ...
  const generateAmortizationSchedule = (view) => {
    ...
    for (let i = 1; i <= numberOfPayments; i++) {
      ...
      const period = view === 'yearly' ? `${Math.ceil(i / 12)} Year` : `${i} Month`
      schedule.push({
        period,
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-white">
      <Card className="w-full max-w-4xl bg-white p-6">
        <CardHeader>
          <CardTitle className="text-2xl font-bold primary-text-gradient">Loan Calculator</CardTitle>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-green-600 font-bold">{formatCurrency(loanAmount)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-gray-700">Total Interest:</span>
              <span className="text-green-600 font-bold">{formatCurrency(totalInterest)}</span>
            </div>
            <div className="flex items-center justify-between">
            ...
            <div className="mt-4">
              <Button variant="outline" className="p-2" onClick={() => setShowAmortization(!showAmortization)}>
                  ...
                  <tbody>
                    {amortizationSchedule.map((entry, index) => (
                      <tr key={index}>
                        ...
                        <td className="border p-2 text-right">{formatCurrency(entry.payment)}</td>
                        <td className="border p-2 text-right">{formatCurrency(entry.principal)}</td>
                        <td className="border p-2 text-right">{formatCurrency(entry.interest)}</td>
                        <td className="border p-2 text-right">{formatCurrency(entry.balance)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
